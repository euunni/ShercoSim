...
int main(int argc, char* argv[]) {
  ...
  RootInterface<DRsimInterface::DRsimEventData>* drInterface = new RootInterface<DRsimInterface::DRsimEventData>("./../../../v250314/ShercoSim/install/input/250327_Generic/ele_" + std::string(filename) + ".root", 1);
  ...
  outEdep.open("./plot/ele_" + filename + "_Edep.csv", std::ios::out | std::ios::app);
  ...
  outHits.open("./plot/ele_" + filename + "_Hits.csv", std::ios::out | std::ios::app);
  ...
  TFile* file = new TFile("./plot/ele_" + filename + ".root", "RECREATE");
  TCanvas* c = new TCanvas("c","");

  c->SetLogy(1);
  tP_leak->Draw("Hist"); c->SaveAs("./plot/ele_" + filename + "_Pleak.png");
  tP_leak_nu->Draw("Hist"); c->SaveAs("./plot/ele_" + filename + "_Pleak_nu.png");
  c->SetLogy(0);

  tEdep->Draw("Hist"); c->SaveAs("./plot/ele_" + filename + "_TotalEdep.png");
  if (doCalib) {
    tE_C->Draw("Hist"); c->SaveAs("./plot/ele_" + filename + "_TotalE_C.png");
    tE_S->Draw("Hist"); c->SaveAs("./plot/ele_" + filename + "_TotalE_S.png");
  }
  tChit->Draw("Hist"); c->SaveAs("./plot/ele_" + filename + "_TotalChit.png");
  tShit->Draw("Hist"); c->SaveAs("./plot/ele_" + filename + "_TotalShit.png");
  tCtime->Draw("Hist"); c->SaveAs("./plot/ele_" + filename + "_TotalCtime.png");
  tStime->Draw("Hist"); c->SaveAs("./plot/ele_" + filename + "_TotalStime.png");
  ...
  if (doCalib) {
    ...
    tE_SC->Draw(""); c->SaveAs("./plot/ele_" + filename + "_TotalE_SC.png");
    ...
    c->SaveAs("./plot/ele_" + filename + "_Ecs.png");
  }
  ...
  tEdep_2D->SetStats(0);
  c->SaveAs("./plot/ele_" + filename + "_Edep2D.pdf");

  c->SetLogz(1);

  tEdep_2D->Draw("COL0Z TEXT"); 
  tEdep_2D->SetStats(0);
  c->SaveAs("./plot/ele_" + filename + "_Edep2D_Log.pdf"); 

  tHits_2D->Draw("COL0Z TEXT"); 
  tHits_2D->SetStats(0);
  c->SaveAs("./plot/ele_" + filename + "_Hits2D_Log.pdf"); 

  if (doCalib) {
    tE_2D->Draw("COL0Z TEXT"); 
    tE_2D->SetStats(0);
    c->SaveAs("./plot/ele_" + filename + "_E2D_Log.pdf"); 
  }
